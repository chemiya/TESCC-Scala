...
*
*  Script para la creación del modelo nº1
*
...
// Usamos el clasificador Gradient-boosted trees

val GBTCensus = new GBTClassifier().
...
/* Entrenamos el modelo: GBT */
val GBTCensusModel_D =GBTCensus.fit(trainCensus)
...
/* Predecimos la clase de los ejemplos de prueba */
val predictionsAndLabelsDF_GBT = GBTCensusModel_D.transform(testCensus).select("prediction", "label")
...
val predictionsGBT = GBTCensusModel_D.transform(testCensus).select("prediction").rdd.map(_.getDouble(0))
val labelsGBT = GBTCensusModel_D.transform(testCensus).select("label").rdd.map(_.getDouble(0))
val accuracy = new MulticlassMetrics(predictionsGBT.zip(labelsGBT)).accuracy
