...
// Validación cruzada y parámetros
val gbt = new GBTClassifier().setLabelCol("label").setFeaturesCol("features")
// Cada una de las siguientes variaciones de paramGrid fue usada en una ejecución diferente del guión, para llegar a los mejores parámetros
// Iteración 1
// val paramGrid = new ParamGridBuilder().addGrid(gbt.maxDepth, Array(3, 10, 17)).addGrid(gbt.maxIter, Array(5, 15, 25)).addGrid(gbt.maxBins, Array(38, 100, 200)).build()
// Iteración 2
// val paramGrid = new ParamGridBuilder().addGrid(gbt.maxDepth, Array(7, 10, 13)).addGrid(gbt.maxIter, Array(10, 15, 20)).addGrid(gbt.maxBins, Array(60, 100, 150)).build()
// Iteración 3
// val paramGrid = new ParamGridBuilder().addGrid(gbt.maxDepth, Array(3, 5, 7)).addGrid(gbt.maxIter, Array(20, 22, 25)).addGrid(gbt.maxBins, Array(38, 50, 60)).build()

// Creamos paramGrid 1x1x1 con los mejores parámetros
val paramGrid = new ParamGridBuilder().addGrid(gbt.maxDepth, Array(5)).addGrid(gbt.maxIter, Array(22)).addGrid(gbt.maxBins, Array(60)).build()
...
// Utilizamos mejores parámetros para crear modelo
val GBTcensus = new GBTClassifier().setFeaturesCol("features").setLabelCol("label").setMaxIter(bestGBTModel.getMaxIter).
...
val GBTcensusModel_D =GBTcensus.fit(trainCensusDFProcesado)
GBTcensusModel_D.write.overwrite().save(PATH + "modeloGBT")