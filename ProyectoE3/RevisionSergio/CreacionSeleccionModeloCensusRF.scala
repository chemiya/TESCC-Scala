...
// Iteración 1
// val paramGrid = new ParamGridBuilder().addGrid(rf.maxDepth, Array(3, 10, 17)).addGrid(rf.numTrees, Array(10, 100, 200)).addGrid(rf.maxBins, Array(38, 100, 200)).build()
// Iteración 2
// val paramGrid = new ParamGridBuilder().addGrid(rf.maxDepth, Array(7, 10, 13)).addGrid(rf.numTrees, Array(60, 100, 150)).addGrid(rf.maxBins, Array(60, 100, 150)).build()
// Iteración 3
// val paramGrid = new ParamGridBuilder().addGrid(rf.maxDepth, Array(9, 10, 11)).addGrid(rf.numTrees, Array(150, 170, 200)).addGrid(rf.maxBins, Array(150, 170, 200)).build()

// Creamos paramGrid 1x1x1 con los mejores parámetros
val paramGrid = new ParamGridBuilder().addGrid(rf.maxDepth, Array(10)).addGrid(rf.numTrees, Array(150)).addGrid(rf.maxBins, Array(170)).build()
...
// Utilizamos mejores parámetros
val RFcensus = new RandomForestClassifier().setFeaturesCol("features").
 setLabelCol("label").
...
val RFcensusModel_D =RFcensus.fit(trainCensusDFProcesado)
RFcensusModel_D.toDebugString
RFcensusModel_D.write.overwrite().save(PATH + "modeloRF")